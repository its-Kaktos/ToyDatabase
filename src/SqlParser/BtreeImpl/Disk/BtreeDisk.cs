...
public class BtreeDisk
{
    ...
    public BtreeDisk(int maxKeysCount)
    {
        ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(maxKeysCount, 2);

        _maxKeysCount = maxKeysCount;
        Root = new BtreeNodeDisk(_maxKeysCount);
    }

    public BtreeNodeDisk Root { get; private set; }
    ...
    public void SetRoot(BtreeNodeDisk node)
    {
        Root = node;
    }
    ...
    private void BalanceTreeAfterDeletion(BtreeNodeDisk node)
    {
    ...
    // Tail recursive calls are never optimized in c#! : https://blog.objektkultur.de/about-tail-recursion-in-.net/
    // You can see it is not optimized in the IL Viewer.
    private void BalanceTree(BtreeNodeDisk node)
    {
        ...
        var parentNode = node.ParentNode ?? new BtreeNodeDisk(_maxKeysCount);

        // From now on, node is the left child.
        node.SetKeys(leftNodeKeys);
        node.ParentNode = parentNode;

        var rightNode = new BtreeNodeDisk(_maxKeysCount, parentNode);
    ...
    private BtreeNodeDisk GetRootNode(BtreeNodeDisk node)
    {
    ...
    private BtreeNodeDisk GetNodeToInsertKey(int key, BtreeNodeDisk node)
    {
        while (true)
        {
    ...
    private BtreeNodeDisk? GetNodeToDeleteKey(int key, BtreeNodeDisk node)
    {
        while (true)
        {
            var (keyIndex, childIndex) = BinarySearchKey(node, key);
    ...
    private int? Search(int key, BtreeNodeDisk node)
    {
        var (keyIndex, childIndex) = BinarySearchKey(node, key);
    ...
    /// </returns>
    private (int? keyIndex, int? childIndex) BinarySearchKey(BtreeNodeDisk node, int target)
    {
        var start = 0;
